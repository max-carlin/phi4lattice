...
from src.action import magnetization
import sys
import numpy as np
import os
import jax
...
class TestAction(unittest.TestCase):
    ...
    def test_constant_field_action_all_rand(self):
        ...
        fill_value = random.uniform(-5.0, 5.0)
    ...
    def test_action_batched_fields_rand(self):
        ...
        for _bs in range(batch_size):
            ...
            U_manual_single = phi_x_single ** 2 + lam * (phi_x_single ** 2 - 1.0) ** 2
            U_manual_single = U_manual_single.sum()
    ...
    def test_action_Z2_symmetry(self):
        ...
        lam = random.uniform(-10.0, 10.0)
        kappa = random.uniform(-10.0, -10.0)
        D = random.randint(1, 5)
        random_seed = random.randint(0, 10000)
        L_array_list = random_int_uniform(n=D,
                                          lower=1,
    ...
    def test_action_translational_invariance_action(self):
        ...
        for d in range(D):
            L_d = L_array[d]
            shift_d = random.randint(1, L_d-1)
            shifts.append(shift_d)
        # pad shifts for non-spatial dims
        total_shifts = [0]*shift + shifts
        phi_x_shifted = jnp.roll(phi_x, shift=total_shifts, axis=tuple(range(phi_x.ndim)))
